...
struct DetailRecapView: View {
    ...
    @State var firebaseVM: FirebaseViewModel = FirebaseViewModel()
    var body: some View {
        ScrollView {
            VStack {
                HStack {
                    VStack (alignment: .leading){
                        ...
                        ScrollView {
                            HStack {
                                ...
                                images.removeAll()
                                Task {
                                    await firebaseVM.clearTripImages(tripID: trip.getName())
                                }
                                        ...
                                        Task {
                                            if let imageData = try? await eachItem.loadTransferable(type: Data.self) {
                                                if let image = UIImage(data: imageData) {
                                                    ...
                                                    firebaseVM.storeImageAndReturnURL(image: image, tripID: trip.getName(), completion: {
                                                        url in
                                                    })
                                                }
                                            }
            ...
            Task {
                ...
                var imageURLs: [String] = await firebaseVM.getAllImages(tripID: trip.getName())
                for image in imageURLs {
                    firebaseVM.getImageFromURL(urlString: image, completion: { uiImage in
